...
| 超级英雄的名字        | 消息          | 发布时间  |
| ------------- |:-------------:| -----:|
| 暴怒船长      | 消息已经发布过了？ | 2012/07/07/07:15 |
| 语文博士      | 应该用“Is”而不是“Has"      |   2012/07/07/07:17 |
| 暴怒船长 | 消息已经发布过了？      |  2012/07/07/07:18 |
| 暴-船 | 消息已经发布过了？      |  2012/07/07/07:19 |
...
###规范化的三个步骤
规范化有助于你更有效地的存储书库。只要模型完全地的规范化处理，他们就不会有冗余的数据，每个模型应该只包含逻辑上关联到自身的数据。  
...
| 名字       | 消息          | 能力 |第一次使用记录（维度，经度，国家，时间） |
| ----------| :-----------:|:----:|----------------------------------:|

  
上面的地理苏剧提取自http://www.golombek.com/locations.html.  
...
如之前所做那样，我们用对应的`User ID`替换了超级英雄的名字，这个用户ID用来引用用户表格。
...
###模型mixins
模型mixins是一个可以把抽象基类当作父类来添加的模型。Python支持多种继承，不像其他的语法，比如Java那样。因此，你可以列出一个模型的任意数量的父类。  
