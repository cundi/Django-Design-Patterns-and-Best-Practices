...
## 增强针对admin的模型
admin应用足够的聪明，因此它可以自动地从模型发现非常多东西。可是，有时候推定信息需要改进。这通常涉及到模型自身添加一个属性或者一个方法（而不是在ModelAdmin类中添加）。  
...
这里建议你定义前面的Meta属性和方法，而不仅仅是只针对于admin接口，而且也是为了在shell中和日志文件中，等等中更好的展现内容。  
